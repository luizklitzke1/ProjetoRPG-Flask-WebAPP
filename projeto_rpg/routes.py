...
def registrar_usuario():
    ...
    if form.validate_on_submit():
        ...
        if form.foto.data:
                picture_file = salvar_imagem('static/imagens_perfil', form.foto.data)
                form.foto.data = picture_file

        novo_usuaio = Usuario(nome=form.nome.data, email=form.email.data, senha=senha_hash, foto = form.foto.data)
    ...
    if usuario_id == current_user.id or current_user.isAdmin == True:
        if form.validate_on_submit():
            ...
            if form.imagem_perfil.data:
                picture_file = salvar_imagem('static/imagens_perfil', form.imagem_perfil.data)
...
def registrar_personagem():
    ...
    if form.validate_on_submit():
        ...
        if form.foto_referencia.data:
            picture_file = salvar_imagem('static/imagens_personagens', form.foto_referencia.data)
